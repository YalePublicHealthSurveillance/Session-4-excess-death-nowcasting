...
nchs.base.comb <- read.csv('https://raw.githubusercontent.com/wpinvestigative/excess_deaths_covid19/master/data/archive/nchs.base.comb_2020-05-29.csv')

#saveRDS(nchs.base.comb,'./Data/nchs_base2_data/2020_05_19.rds')

nchs.base.comb$week_end <- as.Date(nchs.base.comb$week_end)
...
cdc.data <- readRDS('./Data/cdc_covid_data/2020_06_12_11_50.rds')
...
data.vintage <- as.Date('2020-05-29')
...
analysis.data$one <- 1
analysis.data2 <-
    analysis.data[!is.na(analysis.data$nchs.pic),c('week_end','state','nchs.total.deaths','nchs.pic','log.flu.deseasonalize.0','one', 'prop.complete', 'n.weeks.ago')]
analysis.data2 <- analysis.data2[order(analysis.data2$state, analysis.data2$week_end),]

saveRDS(analysis.data2,'./Data/processed_data.rds')
```
...
Alternative plot
```{r}
subs <- nat.range.ac2[nat.range.ac2$week_end>='2020-01-01',]
subs$excess <- subs$obs-subs$pred
plot(subs$week_end, subs$excess, type='l', bty='l', ylab="Deaths", xlab='')
points(subs$week_end, subs$covid.death.hybrid, type='l', col='red')

plot(subs$week_end, subs$covid.death.hybrid/subs$excess, ylim=c(0,1))
```
