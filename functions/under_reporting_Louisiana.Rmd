...
saveRDS(compare.c,'./Data/fluview_archived_data.rds')
...
compare.m.alt.spl2.la <- compare.m.alt.spl2[compare.m.alt.spl2$state=='Louisiana',] 
```


## read in model
```{r}
source('./functions/jags_negbin4_orig.R')
...
saveRDS(reporting.triangle, './outputs/reporting.triangle.louisiana.rds')
...
write.csv(all.preds.df,'./outputs/NobBs.preds.LA.csv')
saveRDS(complete.prop.est.iter.df,'./outputs/NobBs.complete.prop.est.iter.df.LA.rds')
...
write.csv(complete.wk,'./outputs/NobBs.complete.LA.csv')
...
saveRDS(complete.wk.iter,'./outputs/NobBs.complete.iters.LA.rds')
...
probs.est <- as.data.frame(exp(t(apply(res1$Louisiana$probs.samps,2, quantile, probs=c(0.025, 0.5, 0.975)))))
probs.est$weeks.ago <- 2:(nrow(probs.est)+1)
names(probs.est) <- c('lcl','median','ucl','weeks.ago')

saveRDS(probs.est,'./outputs/Nobs.prop.reporting.LA.rds')
