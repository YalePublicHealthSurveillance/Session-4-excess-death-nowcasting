...
e1 <- readRDS('./Data/fluview_archived_data.rds') %>%
  filter(state=='Louisiana' & death_date >= '2020-01-01' )


View(e1)

#write.csv(e1, './outputs/LA_death_archives.csv' )
...
What proportion are reported in each week?. Note this is not quite right because for more recent weeks, we are not observing the full delay distribution

```{r}
proportion <- reporting.triangle %>%
  apply(1, function(x){ x/sum(x, na.rm=T)}) %>% #divide each value by the sum of the row
  t() #transpose
```
...
Number of cases reported for week t, with a delay of d weeks:
...
## Part 3: Harmonic Regression time!
...
d1 <- readRDS('./Data/processed_data.rds') %>%
  filter(state=='LA') %>%
  mutate(deaths.corrected =  nchs.total.deaths/ prop.complete )
...
d1 <- d1 %>%
  arrange(week_end) %>% #sort bydate
  mutate( t = row_number(),
          sin1 = sin(2*pi*t/52.143),
          cos1 = cos(2*pi*t/52.143),
          
          sin2 = sin(2*pi*t*2/52.143),
          cos2 = cos(2*pi*t*2/52.143),
          
          sin3 = sin(2*pi*t*3/52.143),
          cos3 = cos(2*pi*t*3/52.143),
          )
...
```{r}
d1 <- d1 %>%
  mutate( deaths.pre = if_else(d1$week_end < '2020-03-01', nchs.total.deaths, NA_real_ ))
...
## Part 4: Calculating uncertainty: 2-stage Monte Carlo approach
